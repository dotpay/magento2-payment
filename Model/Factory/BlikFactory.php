...
/**
 * Factory which produces SDK BLIK channel.
 */
class BlikFactory extends AbstractFactory
{
    /**
     * Return SDK Blik channel filled by all necessary data.
     *
    ...
    {
        $channel = new Blik($this->paymentAdapter->getConfiguration(),
                          $this->getTransaction(),
                          $this->getPaymentResource(),
                          $this->getSellerResource());
        $channel->setSeller($this->getSeller());
        $channel->setBlikCode($additionalInformation['blik_code']);
