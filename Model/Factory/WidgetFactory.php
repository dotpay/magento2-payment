...
/**
 * Factory which produces SDK channel using standard Dotpay widget.
 */
class WidgetFactory extends AbstractFactory
{
    /**
     * Return SDK Dotpay widget channel filled by all necessary data.
     *
    ...
    {
        $channel = new Dotpay($this->paymentAdapter->getConfiguration(),
                          $this->getTransaction(),
                          $this->getPaymentResource(),
                          $this->getSellerResource());
        if (isset($additionalInformation['channel'])) {
            $channel->setChannelId($additionalInformation['channel']);
        }
